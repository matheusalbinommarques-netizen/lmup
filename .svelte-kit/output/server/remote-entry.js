...
async function run_remote_function(
  event,
  state,
  allow_cookies,
  arg,
...
function command(validate_or_fn, maybe_fn) {
  ...
  const wrapper = (arg) => {
    ...
    const promise = Promise.resolve(
      run_remote_function(event, state, true, arg, validate, fn),
    );
    promise.updates = () => {
      throw new Error(
  ...
  function create_instance(key) {
    ...
    Object.defineProperty(button_props, 'enhance', {
      value: () => {
        return {
          type: 'submit',
          formaction: instance.buttonProps.formaction,
          onclick: () => {},
        };
      },
    });
    ...
    const __ = {
      ...
      fn: async (form_data) => {
        const validate_only =
          form_data.get('sveltekit:validate_only') === 'true';
        ...
        return create_field_proxy(
          ...
          (path, value) => {
            ...
            const input =
              path.length === 0
                ? value
                : deep_set(data?.input ?? {}, path.map(String), value);
    ...
    if (key == void 0) {
      Object.defineProperty(instance, 'for', {
        ...
        value: (key2) => {
          ...
          let instance2 = (state.form_instances ??=
      ...
      if (!state.prerendering && !BROWSER && !event.isRemoteRequest) {
        try {
          return await get_response(__, arg, state, async () => {
            ...
            const promise3 = (cache[key] ??= fetch(
  ...
  const wrapper = (arg) => {
    ...
    promise.set = (value) => {
      ...
      if (__.id) {
        ...
        refreshes[create_remote_cache_key(__.id, key)] = cache[key] =
    ...
    promise.refresh = () => {
      ...
      const cache_key = create_remote_cache_key(
        __.id,
        stringify_remote_arg(arg, state2.transport),
      );
