...
import { a as wt, b as L } from './BO9mVFXQ.js';
import { o as De } from './DcfkiRxm.js';
const V = [];
function ke(e, t = he) {
  let n = null;
  const a = new Set();
...
function Ct() {
  ...
  async function a() {
    ...
    try {
      ...
      return (o && (e(!0), et.v(), clearTimeout(n)), o);
    } catch {
      return !1;
    }
  }
...
let k, Y, me;
const Dt =
  De.toString().includes('$$') || /function \w+\(\) \{\}/.test(De.toString());
Dt
  ? ((k = {
      data: {},
      form: null,
...
const Vt = new Set(['icon', 'shortcut icon', 'apple-touch-icon']),
  q = We(He) ?? {},
  H = We(Ye) ?? {},
  C = { url: Ke({}), page: Ke({}), navigating: ke(null), updated: Ct() };
function Ue(e) {
  q[e] = le();
}
function Bt(e, t) {
...
async function pn(e, t, n) {
  (globalThis.__sveltekit_gmo5ee?.data && globalThis.__sveltekit_gmo5ee.data,
  ...
  (n
    ...
    : (await M({
        type: 'enter',
        url: Xe(v.hash ? on(new URL(location.href)) : location.href),
        replace_state: !0,
      }),
      r()),
    tn());
...
async function zt(e, t, n, a) {
  ...
  (t.invalidateAll && rt(),
    await M({
      ...
      nav_token: a,
      accept: () => {
        (t.invalidateAll && ((Ie = !0), (r = [...Fe.keys()])),
    ...
    if (
      ...
      (R = {
        ...
        promise: ft({ ...e, preload: t }).then(
          (n) => (
            se.delete(t),
            n.type === 'loaded' && n.state.error && rt(),
            n
          ),
        ),
        fork: null,
...
function ie({
  ...
  ((e.pathname = vt(e.pathname, s)), (e.search = e.search));
  const c = {
    type: 'loaded',
    state: { url: e, params: t, branch: n, error: r, route: i },
    props: { constructors: Nt(n).map((u) => u.node.component), page: Ne(k) },
  };
...
function Gt(e, t, n) {
  ...
  const i = oe ? xt(a, r.href, t) : Rt(a, t);
...
async function Ce({ status: e, error: t, url: n, route: a }) {
  ...
  try {
    ...
    if (o instanceof Ee) return zt(new URL(o.location, location.href), {}, 0);
    throw o;
  }
}
async function Xt(e) {
    ...
    const a = (async () => {
      let r =
        (await v.hooks.reroute({
          url: new URL(e),
          fetch: async (i, o) => Gt(i, o, e).promise,
        })) ?? e;
      if (typeof r == 'string') {
        const i = new URL(e);
...
async function fe(e, t) {
  if (e && !ue(e, L, v.hash)) {
    ...
    for (const r of Te) {
      const i = r.exec(a);
      if (i)
        return { id: ce(e), invalidating: t, route: r, params: bt(i), url: e };
    }
  }
}
function Qt(e) {
...
async function M({
  ...
  if (oe) {
    const b = (
      await Promise.all(Array.from(Kt, (P) => P(g.navigation)))
    ...
    const D = S && (await S);
    (D ? (E = D.commit()) : (Oe.$set(d.props), nt(d.props.page), (E = _t?.())),
      (it = !0));
  ...
  if (ze) {
    ...
    if (A) scrollTo(A.x, A.y);
      ...
      A = { x: pageXOffset + P, y: pageYOffset + D };
    } else scrollTo(0, 0);
  }
  const gt =
...
async function ht(e, t, n, a, r) {
  return e.origin === Le && e.pathname === location.pathname && !st
    ? await Ce({ status: a, error: n, url: e, route: t })
    : await J(e, r);
}
function Zt() {
  let e, t, n;
  ...
  const r = new IntersectionObserver(
    (s) => {
      for (const c of s)
        c.isIntersecting && (_e(new URL(c.target.href)), r.unobserve(c.target));
    },
    { threshold: 0 },
  );
  async function i(s, c) {
    const l = Ze(s, O),
      f = l === t && c >= n;
    if (!l || f) return;
    const { url: h, external: w, download: u } = we(l, L, v.hash);
    if (w || u) return;
    const g = te(l),
      _ = h && ce(m.url) === ce(h);
    if (!(g.reload || _))
  ...
  function o() {
    ...
    for (const s of O.querySelectorAll('a')) {
      const { url: c, external: l, download: f } = we(s, L, v.hash);
      if (l || f) continue;
      const h = te(s);
      h.reload ||
        (h.preload_code === j.viewport && r.observe(s),
        h.preload_code === j.eager && _e(c));
    }
  }
  (G.add(o), o());
}
...
function tn() {
  ((history.scrollRestoration = 'manual'),
    addEventListener('beforeunload', (t) => {
      let n = !1;
      if ((Ge(), !X)) {
        const a = je(m, void 0, null, 'leave'),
          r = {
            ...a.navigation,
            cancel: () => {
    ...
    O.addEventListener('click', async (t) => {
      ...
      if (f !== void 0 && h) {
        ...
        if (((z = !0), Ue(y), e(a), !s.replace_state)) return;
        z = !1;
      }
      (t.preventDefault(),
        await new Promise((w) => {
  ...
  for (const t of document.querySelectorAll('link'))
    Vt.has(t.rel) && (t.href = t.href);
  addEventListener('pageshow', (t) => {
    t.persisted && C.navigating.set((Y.current = null));
  });
  function e(t) {
    ((m.url = k.url = t), C.page.set(Ne(k)), C.page.notify());
  }
}
async function nn(
  e,
...
export { pn as a, un as l, k as p, C as s };
